...
baromg <- read.csv("J:/resultados_clusters/simdataset_BarOmega_3_5_2000.txt", 
                   sep= ",", 
                   header = T)
...
baromg %>% ggplot(aes(x = BarOmega, y = RandIndex, color = Cluster)) +
  #geom_line(size=1) +
  theme_classic()+
...
baromg %>% ggplot(aes(x = BarOmega, y = ClassProp, color = Cluster)) +
